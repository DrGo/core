...
type Sessions struct {
	...
	xbuf        SessionID //allocate fixed space
	sid         []byte    //get a slice from it
}
...
func NewSessionManager(cookieName string, maxLifeTime int) *Sessions {
	ss := &Sessions{
		...
		store:       make(map[[32]byte]*Session),
		lastCleaned: time.Now(),
		xbuf:        [32]byte{}, //allocate fixed butffer for session ID generation
	}
	ss.sid = ss.xbuf[:] //get a slice from it
...
//GenSessionID returns a unique session ID (not a UUID but good enough)

func (ss *Sessions) GenSessionID() SessionID {
	// eg https://golang.org/src/math/big/intconv.go?s=864:913#L20
	strconv.AppendInt(ss.sid, time.Now().UnixNano(), 16)
	strconv.AppendInt(ss.sid, rand.Int63(), 16)
	return ss.xbuf
	/*
...
func (ss *Sessions) GetExisting(sessionID [32]byte) *Session {
	ss.RLock()
...
func (ss *Sessions) Prune(maxLifeTime int64) {
	...
	for sid, s := range ss.store {
		if time.Now().Unix() > (s.LastActive.Unix() + maxLifeTime) {
			_ = errors.Debug && errors.Logln("expiring session", s.ID)
			delete(ss.store, sid)
		}
	}
}
